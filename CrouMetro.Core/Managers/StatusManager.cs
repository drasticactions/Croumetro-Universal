...
namespace CrouMetro.Core.Managers
{
    public class StatusManager
    {
        ...
        public static async Task<bool> UpdateStatusWithMedia(String status, String path, IRandomAccessStream fileStream,
            long? inReply, bool? isQuote, bool? trim, UserAccountEntity account)
        {
            try
            {
                ...
                if (account != null)
                {
                    ...
                    {
                        AuthenticationManager authManager = new AuthenticationManager();
                        await authManager.RefreshAccessToken(account);
                    }
                }
